...
```{r}
vec_founder_sire_idx <- sapply(tbl_ped$Sire[!is.na(tbl_ped$Sire)], function(x) !is.element(x, tbl_ped$Calf), USE.NAMES = FALSE)
...
```{r}
vec_founder_dam_idx <- sapply(tbl_ped$Dam[!is.na(tbl_ped$Dam)], function(x) !is.element(x, tbl_ped$Calf), USE.NAMES = FALSE)
